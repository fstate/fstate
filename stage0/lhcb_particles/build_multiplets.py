...
if __name__ == "__main__":
    with open('particles.txt') as f:
        ...
        open('particles.json', 'w').write(json.dumps(particles))
        open('multiplets.json', 'w').write(json.dumps(multiplets))

        # keys = [x for x in multiplets.keys() if len(multiplets[x]) == 1 and x[-1] == '0']

        # for k1 in keys:
        #     if k1[:-1] in multiplets:
        #         print "Looks like we found {} for key {}, merging!".format(k1[:-1], k1)
        #     else:
        #         l = sorted([(k2, len(set(k1) - set(k2))) for k2 in multiplets.keys() if k1 != k2], key=lambda x: x[1])[:3]
        #         print "Nothing found for {}, suggesting: {}".format(k1, l)
